...
def get_weather_forecast(location: str, days: int = 3) -> Dict[str, Any]:
    ...
    forecast = {
        ...
        "forecast_days": days,
        "current": {
            "temperature_c": round(base_temperature, 1),
    ...
    irradiance_factor = {"sunny": 1.0, "partly_cloudy": 0.65, "cloudy": 0.25}
    for hour in range(24):
        ...
        forecast["hourly"].append({
            "hour": hour,
            "temperature_c": round(
                base_temperature + 5 * math.sin(math.pi * (hour - 8) / 12), 1
            ),
            "condition": condition,
            "solar_irradiance": round(850 * daylight * irradiance_factor[condition], 1),
            "humidity": random.randint(35, 80),
            "wind_speed": round(random.uniform(2, 20), 1),
        })
...
def get_electricity_prices(date: str = None) -> Dict[str, Any]:
    ...
    base_price = round(random.uniform(0.12, 0.18), 3)
    hourly_rates = []
    ...
    for hour in range(24):
        ...
        hourly_rates.append({
            "hour": hour,
            "rate": round(base_price * (1.5 if is_peak else 1.0), 3),
            "period": "peak" if is_peak else "off_peak",
            "demand_charge": 0.05 if is_peak else 0,
        })
