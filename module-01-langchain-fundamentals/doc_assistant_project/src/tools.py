...
def create_calculator_tool(logger: ToolLogger):
    ...
    def calculator(expression: str) -> str:
        ...
        Args:
            expression: Mathematical expression using numbers, parentheses, and
                basic operators (+, -, *, /, //, %, **).

        Returns:
            A formatted calculation result or an error message.
        """
        try:
            ...
            if not re.fullmatch(r"[0-9+\-*/().% \t]+", cleaned_expression):
                raise ValueError("Expression contains unsupported characters.")

            result = eval(cleaned_expression, {"__builtins__": {}}, {})
            formatted_result = f"{cleaned_expression} = {result}"
